...
char getKeyCode()
{
  ...
  delay(100);
  keypad.getKey();
  for(char i=1;i<7;i++) //add up all the codes of the pressed keys (1-6 for single keypresses, sum of 2 codes for double key presses, e.g. 3+4=7)
  {
    if(keypad.isPressed(i)) keyCode+=i;
  }
  if(keyCode==lastKey) //if key is still pressed, don't return another
  {
    return NO_KEY;
  }
  else
